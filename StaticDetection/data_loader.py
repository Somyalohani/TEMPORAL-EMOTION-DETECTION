...
def load_data(validation=False, test=False):
    ...
    if DATASET.name == "Fer2013":
        # load train set
        if HYPERPARAMS.features == "landmarks":
        ...
        data_dict['Y'] = np.load(DATASET.train_folder + '/labels.npy').flatten()
        ...
        if validation:
            ...
            if HYPERPARAMS.features == "landmarks":
                ...
                print( "Error '{}' features not recognized".format(HYPERPARAMS.features))
            validation_dict['Y'] = np.load(DATASET.validation_folder + '/labels.npy').flatten()
        ...
        if test:
            ...
            if HYPERPARAMS.features == "landmarks":
                ...
                print( "Error '{}' features not recognized".format(HYPERPARAMS.features))
            test_dict['Y'] = np.load(DATASET.test_folder + '/labels.npy').flatten()
